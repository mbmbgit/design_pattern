...
アルゴリズム（振る舞い）をクラスとして独立させ、実行時に切り替えられるようにするパターンです。クラス本体を変更せずに、ロジック部分だけを部品のように交換・再利用できます。

シナリオ: ECサイトでの支払い処理（クレジットカード払い、Paypal払いなど）。

Q: @abstractmethod はデコレーター？

A: はい、Python の文法上のデコレーターです。

Q: どう機能している？

A: メソッドに「要実装」のタグを付けます。親クラスの ABC がそのタグを見て、「タグが付いたまま（未実装）ならインスタンスを作らせない」という検問を行います。

これにより、チーム開発などで「あ、支払い処理の実装忘れてた！」というミスを、プログラムを実行する前の段階（正確にはオブジェクトを作ろうとした瞬間）で防ぐことができます。
"""
